...
TODO: add axes to a plot without indexing (push!)
      plot(f1, f2) (still not implemented)
...
gnuplot's date support.  The following example is inspired in
(this)[https://lazarusa.github.io/gnuplot-examples/examples/2d/lines/dates].
...
the wrong order) are errors. As an example of this difficulty, consider using
`scatter` to plot some points; we want to use `pointtype` number 4:
...
specifies the point type! To plot a scatter plot using the desired point type,
...
Note how the line color was specified without causing an error, since it is not
