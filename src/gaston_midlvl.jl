...
function addcoords(x::Coord,y::Coord,Z::Array,conf::CurveConf)
    ...
    if neZ
        ...
        @assert length(x) == length(y) "Abscissa and ordinate must have the same number of elements"
    end
    ...
    if nex && !neZ ## 2-d plot
        ...
        @assert validate_2d_plotstyle(conf.plotstyle) "Invalid plotstyle specified"
    elseif nex && neZ ## 3-d plot or image
        @assert validate_3d_plotstyle(conf.plotstyle) "Invalid plotstyle specified"
    end
...
function adderror(yl::Coord,yh::Coord)
    ...
    @assert length(gnuplot_state.figs[c].curves[end].x) == length(yl) "Error data vector must be of same size as abscissa"
    if !isempty(yh)
        @assert length(yh) == length(yl) "Error data vectors must be of same size"
    end
